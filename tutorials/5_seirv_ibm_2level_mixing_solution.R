...
#   --> HEALTH STATES S=0, E=1, I=2, R=3, V=4
...
# load population matrix function
source('misc_demographics.R')
...
# option to disable tertiary cases and estimate R0
track_R0              <- FALSE
...
pop_data      <- get_population_matrix(pop_size)

# set progression parameters: from exposed to infectious
progression_rate <- 1/num_days_exposed
progression_prob <- 1-exp(-progression_rate)  # convert rate to probability
...
pop_data$health[sample(pop_size,pop_size*vaccine_coverage)] <- 4
...
{
  # step 1: identify the exposed and infectious individuals
  ind_exposed     <- which(pop_data$health == 1)
  num_exposed     <- length(ind_exposed)
  
  ind_infectious  <- which(pop_data$health == 2)
  num_infected    <- length(ind_infectious)
  
  
  # step 2: loop over all infected individuals
  p <- ind_infectious[1]
  for(p in ind_infectious)
  {
    ...
    if(track_R0){
      pop_data$health[flag_new_infection] <- 3
    }
  ...
  # step 3a: update the health state of (previously) exposed individuals
  pop_data$health[ind_exposed] <- pop_data$health[ind_exposed] + 
                                  rbinom(num_exposed, size = 1, prob = progression_prob)

  # step 3b: update the health state of (previously) infected individuals
  pop_data$health[ind_infectious] <- pop_data$health[ind_infectious] + 
                                      rbinom(num_infected, size = 1, prob = recovery_prob)
...
log_e <- colSums(log_pop_data == 1)  / pop_size
log_i <- colSums(log_pop_data == 2)  / pop_size
log_r <- colSums(log_pop_data == 3)  / pop_size
log_v <- colSums(log_pop_data == 4)  / pop_size
...
lines(log_e,  col=5,lwd=2)

legend('right',legend=c('S','E','I','R','V'),col=c(1,5,2:4),lwd=2)
